...
_T5_PROCESSES = {
    "로스팅":  {"sensors":["temperature_c","pressure_bar","smoke_adc"],
               "color":"#f0a060", "normal":{"temperature_c":(175,232),"pressure_bar":(0.85,1.45),"smoke_adc":(0,2800)}},
    "분쇄":   {"sensors":["temperature_c","vibration_hz","particle_size_um"],
               "color":"#60c0f0", "normal":{"temperature_c":(38,65),"vibration_hz":(25,92),"particle_size_um":(14,36)}},
    "콘칭":   {"sensors":["temperature_c","viscosity_cp","humidity_pct"],
               "color":"#a0e090", "normal":{"temperature_c":(57,83),"viscosity_cp":(1200,9500),"humidity_pct":(0,44)}},
    "템퍼링": {"sensors":["temperature_c","viscosity_cp"],
               "color":"#e070e0", "normal":{"temperature_c":(20,56),"viscosity_cp":(3000,8500)}},
    "몰딩":   {"sensors":["temperature_c","pressure_bar","humidity_pct"],
               "color":"#f0e060", "normal":{"temperature_c":(27,33),"pressure_bar":(0.2,0.62),"humidity_pct":(22,60)}},
...
_T5_SENSOR_LABELS = {
    ...
    "smoke_adc":"연기 ADC", "vibration_hz":"진동 (Hz)",
    "particle_size_um":"입자크기 (μm)", "viscosity_cp":"점도 (cP)",
...
with tab_train_sensor:
    ...
    if not _SKLEARN_OK:
        ...
        if not t5_uploaded:
            ...
            # ── 제품 × 공정_센서 와이드 포맷으로 피벗 (결측치 처리 적용 후 수행)
            _t5_pivot_rows = []
            ...
            if not t5_selected_models:
                ...
                if st.button("▶ 학습 시작", key="t5_train_btn"):
                    ...
                    st.session_state["t5_FEATURES"]   = _t5_feat_use
          ...
          if not t6_csv:
            ...
            try:
                ...
                if st.button("▶ 예측 시작", key="t6_predict_btn"):
                    with st.spinner("예측 실행 중..."):
                        ...
                        st.session_state["t6_pred_cache"] = {
                            ...
                            "importances":  getattr(_t6_clf, "feature_importances_", None),
                ...
                if "t6_pred_cache" not in st.session_state:
                    ...
                    if _t6_importances is None:
                        st.info("ℹ️ 이 모델(SVM/LogisticRegression/MLP 등)은 피처 중요도를 제공하지 않아 공정·센서 영향 분석을 표시할 수 없습니다.")
                        _t6_present_labels = []
                    ...
                    for _dlbl in _t6_present_labels:
                        ...
                        # "정상" 기준값: NoDefects 샘플이 있으면 그 평균, 없으면 나머지 전체 평균으로 대체
                        _normal_mask = _t6_X_df["_basis_label"] == "NoDefects"
                        _other_mean  = (
                            _t6_X_df.loc[_normal_mask, _t6_feature_cols].mean()
                            if _normal_mask.any()
                            else _t6_X_df.loc[~_grp_mask, _t6_feature_cols].mean()
                        )
        ...
        try:
            ...
            if _pimp7 is None:
                _st7.append(Paragraph(
                    "이 모델(SVM/LogisticRegression/MLP 등)은 피처 중요도를 제공하지 않아 "
                    "공정·센서 영향 분석을 표시할 수 없습니다.", S7_BODY))
                _defect_lbls7 = []
            ...
            for _dlbl7 in _defect_lbls7:
                ...
                # "정상 평균" 기준값: NoDefects 샘플이 있으면 그 평균, 없으면 나머지 전체 평균으로 대체
                _normal_mask7 = _X7_df["_lbl"] == "NoDefects"
                _omean7    = (
                    _X7_df.loc[_normal_mask7, _pfc7].mean()
                    if _normal_mask7.any()
                    else _X7_df.loc[~_gmask7, _pfc7].mean()
                )
